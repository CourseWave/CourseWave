...
exports.addCourseVideos = async (req, res) => {
  try {
    // Ensure that uploadVideoLinks middleware processes the files before proceeding
...
exports.getSectionVideoDetails = async (req, res) => {
  try {
    const video_link = section_videosModel.getSectionVideoDetails.video_link;
    const video_url = video_link ? `http://localhost:5000/${video_url}` : null;
...
exports.getSectionVideos = async (req, res) => {
  try {
    const video_link = section_videosModel.getSectionVideos.video_link;
    const video_url = video_link ? `http://localhost:5000/${video_url}` : null;
