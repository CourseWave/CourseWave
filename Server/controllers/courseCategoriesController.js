...
exports.createCetegory = async (req, res) => {
  try {
    ...
    const newCategoryId = await courseCategoriesModel.addCatagory(category_name);
    res.status(201).json({
...
exports.updateCategory = async (req, res) => {
  try {
    ...
    const { category_name } = req.body;
    const updatedCategory = await courseCategoriesModel.updateCategory(category_id, category_name);
    res.status(200).json({
...
exports.deleteCategory = async (req, res) => {
  try {
    const { category_id } = req.params;
    const deletedCategory = await courseCategoriesModel.deleteCatagory(category_id);
    res.status(200).json({
