...
export const fetchLiveSessions = createAsyncThunk(
  ...
  async () => {
    try {
      ...
      throw Error(
        error.response.data.error || "Failed to retrieve live sessions"
      );
    }
  }
);
  ...
  async (sessionData) => {
    try {
      const response = await axios.post(
        "http://localhost:5000/createLiveSession",
        sessionData
      );
      return response.data;
    } catch (error) {
...
export const updateLiveSession = createAsyncThunk(
  "liveSessions/update",
  async ({ session_id, sessionUpdates }) => {
    try {
      const response = await axios.put(
        `http://localhost:5000/updateLiveSession/${session_id}`,
        sessionUpdates
      );
      return response.data;
    } catch (error) {
  ...
  async (session_id) => {
    try {
      const response = await axios.delete(
        `http://localhost:5000/deleteLiveSession/${session_id}`
      );
      return response.data;
    } catch (error) {
    ...
    builder
      ...
      .addCase(updateLiveSession.fulfilled, (state, action) => {
        ...
        const index = state.liveSessions.findIndex(
          (session) => session.id === action.payload.updatedSession.id
        );
        if (index !== -1) {
      ...
      .addCase(deleteLiveSession.fulfilled, (state, action) => {
        ...
        state.liveSessions = state.liveSessions.filter(
          (session) => session.id !== action.payload.deletedSession.id
        );
      })
